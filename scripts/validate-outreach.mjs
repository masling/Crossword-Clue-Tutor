...
export async function validateOutreach() {
  ...
  for (const item of manifest.items ?? []) {
    ...
    if (item.channel === "email") {
      ...
      if (!new Set(["pending", "pending_phone_omitted", "submitted_no_receipt", "submitted_user_confirmed", "blocked_recaptcha"]).has(item.submissionStatus ?? "pending")) errors.push(`${item.id} has an unsupported form status`);
      ...
      if (new Set(["pending", "pending_phone_omitted"]).has(item.submissionStatus ?? "pending") && item.submittedAt !== null) errors.push(`${item.id} is unexpectedly marked submitted`);
