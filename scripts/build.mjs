...
const classroomSkillLabels = { "context-and-meaning": "Context and meaning", "word-structure": "Word structure", "academic-language": "Academic language", "science-vocabulary": "Science vocabulary", "language-arts": "Language arts", "precision-and-revision": "Precision and revision" };
...
const classroomExamples = Object.entries(classroomSkillLabels).map(([skill, label]) => {
...
const classroomPackLinks = Object.entries(classroomSkillLabels).map(([skill, label]) => `<a href="${classroomPackHubRoute}${skill}/"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(classroomPackDetails[skill].summary)}</span><small>5 original clues · student sheet + teacher key</small><b aria-hidden="true">→</b></a>`).join("");
const classroomPackHubBody = `<section class="shell page-hero">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Teacher guide", href: vocabularyGuideRoute }, { label: "Classroom skill packs" }])}<p class="content-kind">Ready-to-print · grades 6–12</p><h1>Six crossword vocabulary practice packs</h1><p>Each pack contains five original clues, an evidence-writing space, difficulty labels, and a separate teacher key. Use one pack for a short lesson or combine packs for review.</p></section><section class="shell classroom-pack-list">${classroomPackLinks}</section>`;
const classroomPackHubLd = { "@context": "https://schema.org", "@type": "CollectionPage", name: "Crossword vocabulary classroom practice packs", description: "Six printable classroom vocabulary packs with original crossword clues, student evidence prompts, and teacher keys.", url: canonicalUrl(classroomPackHubRoute), mainEntity: { "@type": "ItemList", itemListElement: Object.entries(classroomSkillLabels).map(([skill, label], index) => ({ "@type": "ListItem", position: index + 1, name: label, url: canonicalUrl(`${classroomPackHubRoute}${skill}/`) })) } };
await writePage(classroomPackHubRoute, pageTemplate({ title: "Printable crossword vocabulary practice packs", description: "Print six Grades 6–12 crossword vocabulary practice packs with original clues, evidence prompts, and separate teacher keys.", route: classroomPackHubRoute, body: classroomPackHubBody, bodyClass: "classroom-pack-hub-page", jsonLd: [classroomPackHubLd], analyticsMode: "minimal" }), false, "2026-08-24");

for (const [skill, label] of Object.entries(classroomSkillLabels)) {
