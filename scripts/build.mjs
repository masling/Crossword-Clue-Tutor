...
for (const type of clueTypes) {
  ...
  const pageTitle = isAbbreviationGuide ? "Crossword abbreviations: clue signals and answer formats" : `${type.title}: how they work`;
  const description = isAbbreviationGuide ? "Learn common crossword abbreviation signals, answer formats, map directions, initialisms, and reviewed examples." : `${type.summary} See common signals, a worked example, and practical solving advice.`;
  const lastmod = matchingClues.map((clue) => clue.reviewedAt).sort().reverse()[0] ?? config.contentUpdatedAt;
