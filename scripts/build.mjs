...
for (const type of clueTypes) {
  ...
  const pageHeading = isAbbreviationGuide ? "Crossword abbreviations and clue signals" : isQuestionMarkGuide ? "What does a question mark mean in a crossword clue?" : type.title;
  ...
  const pageTitle = isAbbreviationGuide ? "Crossword abbreviations: clue signals and answer formats" : isQuestionMarkGuide ? "Question mark crossword clues: what the ? means" : `${type.title}: how they work`;
  const description = isAbbreviationGuide ? "Learn common crossword abbreviation signals, answer formats, map directions, initialisms, and reviewed examples." : isQuestionMarkGuide ? "Learn why crossword clues use question marks, how they signal wordplay and misdirection, and see reviewed examples." : `${type.summary} See common signals, a worked example, and practical solving advice.`;
