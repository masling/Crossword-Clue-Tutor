...
const aboutBody = `<article class="shell prose-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "About" }])}<h1>Less spoiling. More solving.</h1><p>Clue Tutor is an independent crossword learning tool. It helps solvers use crossing letters, notice clue signals, and understand why an answer fits instead of dropping an unexplained answer at the top of the page.</p><h2>What we are building</h2><p>A fast pattern-aware solver, a clear “explain my answer” workflow, and an editorial library of crosswordese and clue-reading guides.</p><h2>What we are not</h2><p>We are not affiliated with any crossword publisher, and we do not reproduce complete commercial puzzles or present automatically generated text as reviewed guidance.</p><section id="contact"><h2>Contact</h2><p>Use the <a href="/feedback/">feedback form</a> for a wrong answer, missing possibility, unclear explanation, or tool problem. For editorial questions, directory listings, or a conversation that needs a reply, email <a href="mailto:${contactEmail}">${contactEmail}</a>.</p><p>Clue Tutor does not operate a marketing mailing list. Messages are used only to handle the request or conversation you start.</p></section></article>`;
const aboutLd = { "@context": "https://schema.org", "@type": "AboutPage", name: "About Crossword Clue Tutor", url: canonicalUrl("/about/"), mainEntity: { "@id": organizationId } };
await writePage("/about/", pageTemplate({ title: "About", description: "Why Clue Tutor puts progressive hints and clear explanations ahead of instant crossword spoilers, plus how to contact the independent site.", route: "/about/", body: aboutBody, bodyClass: "prose-page-body", jsonLd: [aboutLd] }), false, "2026-08-20");
