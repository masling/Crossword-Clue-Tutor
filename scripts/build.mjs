...
const [config, clues, answers, clueTypes, publications, clueHubs, classroomClues] = await Promise.all([
...
function seoDescription(subject, suffix) {
  const separator = " — ";
  const maxLength = 160;
  const subjectBudget = maxLength - separator.length - suffix.length;
  const cleanSubject = subject.trim();
  const fittedSubject = cleanSubject.length <= subjectBudget
    ? cleanSubject
    : `${cleanSubject.slice(0, Math.max(1, subjectBudget - 1)).trimEnd()}…`;
  return `${fittedSubject}${separator}${suffix}`;
}
  ...
  return `<footer class="site-footer">
    <div class="shell footer-grid">
      ...
      <nav aria-label="Footer navigation">
        ...
        <a href="/content-license/">Content &amp; dataset license</a>
        ...
        <a href="/educators/crossword-vocabulary-pilot/">Teacher pilot kit</a>
        <a href="/educators/crossword-vocabulary-worksheet/">Printable worksheet</a>
        <a href="/educators/crossword-vocabulary-packs/">Classroom skill packs</a>
        <a href="/research/classroom-crossword-vocabulary-corpus/">Classroom corpus</a>
...
function pageTemplate({ title, description, route, body, bodyClass = "", noindex = false, jsonLd = [], analyticsMode = "standard" }) {
  ...
  const adsenseAccountMarkup = adsense?.clientId
    ? `<meta name="google-adsense-account" content="${escapeHtml(adsense.clientId)}">`
    : "";
  const adsenseMarkup = !noindex && analyticsMode === "standard" && route !== "/privacy/" && adsense?.clientId && adsense?.scriptSrc
    ? `<script async src="${escapeHtml(adsense.scriptSrc)}?client=${escapeHtml(adsense.clientId)}" crossorigin="anonymous"></script>`
    : "";
...
const contentLicenseRoute = "/content-license/";
const contentLicenseVersion = "1.0";
const contentLicenseDate = "2026-08-29";
const classroomCorpusLicense = { "@type": "CreativeWork", name: `Crossword Clue Tutor Classroom Corpus License ${contentLicenseVersion}`, url: canonicalUrl(`${contentLicenseRoute}#classroom-corpus`) };
const ambiguityDatasetLicense = { "@type": "CreativeWork", name: `Crossword Clue Tutor Ambiguity Dataset License ${contentLicenseVersion}`, url: canonicalUrl(`${contentLicenseRoute}#ambiguity-dataset`) };
const contentLicenseBody = `<article class="shell prose-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Content and dataset license" }])}<header><h1>Content and dataset license</h1><p>Version ${contentLicenseVersion} · effective ${escapeHtml(formatDate(contentLicenseDate))}</p></header><p>The MIT License applies to the software source code only. Original clues, hints, definitions, explanations, research datasets, guides, copy, and brand assets remain copyright © 2026 Crossword Clue Tutor contributors unless a specific file states otherwise.</p><section id="classroom-corpus"><h2>Classroom Corpus License</h2><p>Teachers may print and use the Classroom Crossword Vocabulary Corpus in their own classrooms without charge. Researchers, educators, and writers may quote limited examples for teaching, commentary, or reference when they clearly credit Crossword Clue Tutor and include a visible, crawlable standard follow link to the <a href="/research/classroom-crossword-vocabulary-corpus/">Classroom Corpus page</a>.</p><p>This permission does not include bulk republication of the JSON or CSV files, removal of attribution, resale, commercial database mirroring, creation of a competing answer archive, or AI training. Contact <a href="mailto:${contactEmail}">${contactEmail}</a> for broader reuse permission.</p></section><section id="ambiguity-dataset"><h2>Ambiguity Dataset License</h2><p>The Ambiguous Crossword Clues dataset may be downloaded for personal study, classroom discussion, and non-commercial research. Limited excerpts may be quoted with clear credit and a visible, crawlable standard follow link to the <a href="/research/ambiguous-crossword-clues/">Ambiguity Dataset page</a>.</p><p>Bulk redistribution, resale, commercial mirroring, removal of attribution, competing answer archives, and AI training are not granted without prior written permission.</p></section><h2>General boundaries</h2><p>Nothing in this notice limits fair use, fair dealing, or other independently applicable rights. Short publisher clue excerpts, publication names, trademarks, titles, people, characters, and other third-party material remain the property of their respective owners; Crossword Clue Tutor claims rights only in its independently created selection, organization, annotations, explanations, and original classroom material.</p><p>Links marked <code>nofollow</code> or <code>sponsored</code>, hidden behind JavaScript or redirect masking, or blocked from search crawlers do not satisfy the attribution condition.</p></article>`;
const contentLicenseLd = { "@context": "https://schema.org", "@type": "WebPage", name: "Crossword Clue Tutor content and dataset license", url: canonicalUrl(contentLicenseRoute), version: contentLicenseVersion, datePublished: contentLicenseDate, dateModified: contentLicenseDate, copyrightNotice: "Copyright © 2026 Crossword Clue Tutor contributors" };
await writePage(contentLicenseRoute, pageTemplate({ title: "Content and dataset license", description: "Reuse terms for Crossword Clue Tutor classroom materials, research datasets, editorial content, and software source code.", route: contentLicenseRoute, body: contentLicenseBody, bodyClass: "prose-page-body", jsonLd: [contentLicenseLd], analyticsMode: "minimal" }), false, contentLicenseDate);
...
const ambiguityResearchBody = `<article class="shell prose-page research-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Ambiguous clue data" }])}<p class="content-kind">Original editorial dataset · updated ${escapeHtml(formatDate(clueHubLatestReview))}</p><h1>Ambiguous crossword clues by answer length and meaning</h1><p>A single crossword clue can support several correct answers. This living dataset records <strong>${clueHubs.length} recurring clues</strong>, <strong>${ambiguityCandidateCount} reviewed clue-answer possibilities</strong>, and <strong>${ambiguityUniqueAnswers} unique answers</strong> from ${ambiguityMinLength} to ${ambiguityMaxLength} letters. It is designed for solvers, educators, and researchers studying why answer length and clue sense matter.</p><p><a class="button button-primary" href="/assets/clue-hubs.json" download>Download JSON</a> <a class="button button-outline" href="/assets/clue-hubs.csv" download>Download CSV</a></p><h2>Current coverage</h2><div class="research-table-wrap"><table><thead><tr><th scope="col">Clue</th><th scope="col">Candidates</th><th scope="col">Length range</th><th scope="col">Why it is ambiguous</th></tr></thead><tbody>${ambiguityResearchRows}</tbody></table></div><h2>Method</h2><ol><li>Choose a short recurring clue that has several materially different senses.</li><li>Collect plausible answers by sense and letter count without copying a publisher grid or answer list.</li><li>Write a concise definition for the exact relationship between the clue and each answer.</li><li>Editorially review spelling, length, sense, and duplicate entries before publication.</li></ol><h2>What the data does not claim</h2><p>This is a curated teaching dataset, not a frequency ranking and not an exhaustive archive of every published crossword. Candidate order does not predict the answer to a specific puzzle. The grid, crossing letters, grammar, and publisher style still determine the intended fill.</p><h2>License and citation</h2><p>Link to this page as “Crossword Clue Tutor, Ambiguous crossword clues by answer length and meaning” and include the access date. Personal study, classroom discussion, non-commercial research, and limited attributed excerpts are covered by the <a href="${contentLicenseRoute}#ambiguity-dataset">Ambiguity Dataset License ${contentLicenseVersion}</a>.</p><p><a class="text-link" href="/guides/answer-length-and-crossings/">Read the practical ambiguity-solving guide →</a></p><p class="source-note">All descriptions are independently written for Clue Tutor. The dataset does not reproduce complete grids, daily puzzles, or publisher answer keys.</p></article>`;
const ambiguityResearchLd = { "@context": "https://schema.org", "@type": "Dataset", name: "Ambiguous crossword clues by answer length and meaning", description: `A curated dataset of ${ambiguityCandidateCount} reviewed clue-answer possibilities for ${clueHubs.length} recurring ambiguous crossword clues, organized by meaning and answer length.`, url: canonicalUrl(ambiguityResearchRoute), identifier: canonicalUrl(ambiguityResearchRoute), datePublished: "2026-08-20", dateModified: contentLicenseDate, creator: { "@type": "Organization", name: config.name, url: config.siteUrl }, license: ambiguityDatasetLicense, copyrightNotice: "Copyright © 2026 Crossword Clue Tutor contributors", isAccessibleForFree: true, keywords: ["crossword clues", "ambiguous clues", "answer length", "word meaning"], measurementTechnique: "Independent editorial review of clue sense, answer spelling, and letter count", variableMeasured: ["clue", "answer", "answer length", "sense"], distribution: [{ "@type": "DataDownload", contentUrl: canonicalUrl("/assets/clue-hubs.json"), encodingFormat: "application/json", license: ambiguityDatasetLicense }, { "@type": "DataDownload", contentUrl: canonicalUrl("/assets/clue-hubs.csv"), encodingFormat: "text/csv", license: ambiguityDatasetLicense }] };
await writePage(ambiguityResearchRoute, pageTemplate({ title: "Ambiguous crossword clue dataset by length and meaning", description: `Explore and download ${ambiguityCandidateCount} reviewed answers for recurring ambiguous crossword clues, organized by letter count and meaning.`, route: ambiguityResearchRoute, body: ambiguityResearchBody, bodyClass: "prose-page-body research-page-body", jsonLd: [ambiguityResearchLd] }), false, contentLicenseDate);

const classroomCorpusRoute = "/research/classroom-crossword-vocabulary-corpus/";
const classroomCorpusConcepts = new Set(classroomClues.map((item) => item.conceptId)).size;
const classroomCorpusSkills = [...new Set(classroomClues.map((item) => item.skill))].sort();
const classroomCorpusLatestReview = classroomClues.map((item) => item.reviewedAt).sort().at(-1) ?? config.contentUpdatedAt;
const classroomCorpusCsv = [
  "clue,answer,length,skill,subject,difficulty,part_of_speech,hint,definition,signal,explanation,grade_bands,concept_id,source_kind,reviewed_at,content_version",
  ...classroomClues.map((item) => [item.clue, item.answer, item.answer.length, item.skill, item.subject, item.difficulty, item.partOfSpeech, item.hint, item.definition, item.signal, item.explanation, (item.gradeBands ?? []).join("|"), item.conceptId, item.sourceKind, item.reviewedAt, item.contentVersion].map(csvCell).join(","))
].join("\n") + "\n";
const classroomCorpusSkillRows = classroomCorpusSkills.map((skill) => {
  const items = classroomClues.filter((item) => item.skill === skill);
  const label = skill.split("-").map((part) => part[0].toUpperCase() + part.slice(1)).join(" ");
  const advanced = items.filter((item) => item.difficulty === "advanced").length;
  return `<tr><th scope="row"><a href="/educators/crossword-vocabulary-packs/${skill}/">${escapeHtml(label)}</a></th><td>${items.length}</td><td>${advanced}</td><td>Grades 6–8 and 9–12</td></tr>`;
}).join("");
const classroomCorpusBody = `<article class="shell prose-page research-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Classroom crossword corpus" }])}<p class="content-kind">Original classroom dataset · reviewed ${escapeHtml(formatDate(classroomCorpusLatestReview))}</p><h1>Classroom crossword vocabulary corpus</h1><p>Download the same reviewed classroom data used by the hint-first solver and printable skill packs: <strong>${classroomClues.length} original clues</strong>, <strong>${classroomCorpusConcepts} unique concepts</strong>, and <strong>${classroomCorpusSkills.length} curriculum skill areas</strong> for Grades 6–12.</p><p><a class="button button-primary" href="/assets/classroom-clues.json" download>Download JSON</a> <a class="button button-outline" href="/assets/classroom-clues.csv" download>Download CSV</a></p><h2>Fields in each record</h2><p>Each entry includes the classroom clue, reviewed answer, answer length, skill, subject, difficulty, part of speech, hint, definition, clue signal, explanation, grade bands, stable concept identifiers, source category, review date, and content version.</p><h2>Coverage by skill</h2><div class="research-table-wrap"><table><thead><tr><th scope="col">Skill</th><th scope="col">Clues</th><th scope="col">Advanced</th><th scope="col">Grade bands</th></tr></thead><tbody>${classroomCorpusSkillRows}</tbody></table></div><h2>How the corpus is reviewed</h2><ol><li>Select a real curriculum concept and assign stable concept and variant identifiers.</li><li>Write the clue, hint, definition, signal, and explanation independently.</li><li>Reject answer leakage, duplicate pairs, repeated templates, invented terms, incomplete metadata, and sensitive content outside the product boundary.</li><li>Run exact worksheet tests and independently worded engineering hold-outs before reporting data readiness.</li></ol><h2>What the data can and cannot show</h2><p>The corpus establishes a bounded set of reviewed classroom concepts. It does not claim that arbitrary teacher-written clues will always match, and engineering hold-outs do not substitute for independent classroom evidence. The <a href="/educators/crossword-vocabulary-pilot/">two-session teacher pilot</a> collects that evidence separately.</p><h2>Classroom and research use</h2><p>Teachers may print and use these original materials in their own classrooms. Researchers and writers may quote limited examples with clear attribution and a standard follow link to this page. Bulk republication, removal of attribution, or commercial database mirroring is not granted; contact <a href="mailto:${contactEmail}">${contactEmail}</a> for broader reuse.</p><p>These terms are formalized in the <a href="${contentLicenseRoute}#classroom-corpus">Classroom Corpus License ${contentLicenseVersion}</a>.</p><h2>Source boundary</h2><p>Reference sources inform terminology and factual review. Student-facing wording is independently written. This dataset contains no publisher grid, full commercial puzzle, mailbox record, feedback response, or student data.</p><p class="source-note">Suggested citation: Crossword Clue Tutor, “Classroom crossword vocabulary corpus,” version ${classroomCorpusLatestReview}, accessed on your download date.</p></article>`;
const classroomCorpusLd = { "@context": "https://schema.org", "@type": "Dataset", name: "Classroom crossword vocabulary corpus", description: `${classroomClues.length} original reviewed Grades 6–12 crossword vocabulary clues across ${classroomCorpusSkills.length} curriculum skill areas.`, url: canonicalUrl(classroomCorpusRoute), identifier: canonicalUrl(classroomCorpusRoute), datePublished: "2026-08-24", dateModified: contentLicenseDate, version: classroomCorpusLatestReview, creator: { "@type": "Organization", name: config.name, url: config.siteUrl }, license: classroomCorpusLicense, copyrightNotice: "Copyright © 2026 Crossword Clue Tutor contributors", isAccessibleForFree: true, keywords: ["crossword vocabulary", "Grades 6–12", "classroom clues", "vocabulary instruction"], educationalLevel: "Grades 6–12", measurementTechnique: "Independent clue writing, metadata validation, editorial review, leakage checks, duplicate checks, template checks, and retrieval evaluation", variableMeasured: ["clue", "answer", "answer length", "skill", "subject", "difficulty", "part of speech", "hint", "definition", "signal", "explanation", "grade bands", "concept ID", "review date"], distribution: [{ "@type": "DataDownload", contentUrl: canonicalUrl("/assets/classroom-clues.json"), encodingFormat: "application/json", license: classroomCorpusLicense }, { "@type": "DataDownload", contentUrl: canonicalUrl("/assets/classroom-clues.csv"), encodingFormat: "text/csv", license: classroomCorpusLicense }] };
await writePage(classroomCorpusRoute, pageTemplate({ title: "Classroom crossword vocabulary corpus", description: `Explore and download ${classroomClues.length} original reviewed Grades 6–12 crossword vocabulary clues across ${classroomCorpusSkills.length} curriculum skill areas.`, route: classroomCorpusRoute, body: classroomCorpusBody, bodyClass: "prose-page-body research-page-body", jsonLd: [classroomCorpusLd], analyticsMode: "minimal" }), false, contentLicenseDate);
...
const vocabularyGuideLd = { "@context": "https://schema.org", "@type": "Article", headline: "How to use crossword clues for vocabulary learning", description: vocabularyGuideDescription, datePublished: "2026-08-22", dateModified: "2026-08-24", mainEntityOfPage: canonicalUrl(vocabularyGuideRoute), author: organizationEntity, publisher: organizationEntity, educationalLevel: "Grades 6–12", learningResourceType: "Lesson plan" };
const vocabularyGuideEnhancedBody = vocabularyGuideBody
  .replace(`<h2>A 20-minute classroom routine</h2>`, `<p>The <a href="/classroom-solver/">classroom solver</a> includes a growing library of original, reviewed practice clues across language, academic, mathematics, science, and social studies skills. Every clue includes a hint, exact definition, clue signal, and explanation.</p><h2>A 20-minute classroom routine</h2>`)
  .replace(`<p><a class="button button-primary" href="/solver/">Open the classroom-ready solver</a> <a class="button button-outline" href="/research/ambiguous-crossword-clues/">Download the reviewed teaching dataset</a></p>`, `<p><a class="button button-primary" href="/classroom-solver/">Open the reduced-analytics classroom solver</a> <a class="button button-outline" href="/educators/crossword-vocabulary-worksheet/">Print the student worksheet</a></p><p><a class="text-link" href="/educators/crossword-vocabulary-packs/">Print ready-to-use classroom skill packs →</a><br><a class="text-link" href="${classroomCorpusRoute}">Download the 500-clue classroom corpus →</a></p>`);
await writePage(vocabularyGuideRoute, pageTemplate({ title: "Crossword clues for vocabulary learning", description: vocabularyGuideDescription, route: vocabularyGuideRoute, body: vocabularyGuideEnhancedBody, bodyClass: "prose-page-body guide-page", jsonLd: [vocabularyGuideLd], analyticsMode: "minimal" }), false, "2026-08-24");

const classroomSolverRoute = "/classroom-solver/";
const classroomSkillLabels = { "context-and-meaning": "Context and meaning", "word-structure": "Word structure", "academic-language": "Academic language", "science-vocabulary": "Science vocabulary", "language-arts": "Language arts", "precision-and-revision": "Precision and revision", "mathematics-language": "Mathematics language", "social-studies": "Social studies", "digital-literacy": "Digital literacy", "media-literacy": "Media literacy", "financial-literacy": "Financial literacy", "environmental-literacy": "Environmental literacy", "grammar-and-usage": "Grammar and usage", "biology-vocabulary": "Biology vocabulary", "chemistry-vocabulary": "Chemistry vocabulary", "earth-science": "Earth science", "geometry-language": "Geometry language", "statistics-and-data": "Statistics and data", "physics-vocabulary": "Physics vocabulary", "geography-language": "Geography language", "civics-and-government": "Civics and government", "reading-analysis": "Reading analysis", "algebra-language": "Algebra language", "world-history": "World history", "economics-vocabulary": "Economics vocabulary", "ecology-vocabulary": "Ecology vocabulary", "writing-and-rhetoric": "Writing and rhetoric", "computer-science": "Computer science", "health-and-wellness": "Health and wellness" };
const classroomPackDetails = {
  "context-and-meaning": { summary: "Use surrounding evidence and meaning relationships to distinguish literal meaning, association, similarity, opposition, and inference.", teacherMove: "Ask students to underline the wording that identifies the relationship before proposing a fill." },
  "word-structure": { summary: "Use roots, affixes, morphemes, and compounds to make word structure visible.", teacherMove: "Have students divide an example word into meaningful parts after each answer is discussed." },
  "academic-language": { summary: "Translate common assignment verbs and evidence terms into precise actions students can perform.", teacherMove: "After solving, ask students to rewrite one classroom direction using the answer." },
  "science-vocabulary": { summary: "Practice concise definitions from biology, chemistry, physics, ecology, and experimental design.", teacherMove: "Require one feature that separates the answer from a related science term." },
  "language-arts": { summary: "Distinguish figurative language, narrative voice, theme, and irony through exact definitions.", teacherMove: "Ask students to create one original example after defending the answer." },
  "precision-and-revision": { summary: "Use evidence, reliability, exact wording, revision, and justification to strengthen academic responses.", teacherMove: "Have students improve a vague sentence using the solved term as the revision goal." },
  "mathematics-language": { summary: "Clarify the academic words used to describe operations, relationships, geometry, data, and mathematical reasoning.", teacherMove: "Ask students to pair each solved term with a symbolic, numerical, or visual example." },
  "social-studies": { summary: "Practice precise terms from civics, economics, geography, and historical reasoning.", teacherMove: "Have students connect each answer to a concrete historical, civic, economic, or geographic example." },
  "digital-literacy": { summary: "Practice the vocabulary needed to navigate digital systems, protect information, and evaluate online tools responsibly.", teacherMove: "Ask students to connect each answer to a safe, realistic action they would take on a device or online account." },
  "media-literacy": { summary: "Identify the language of sourcing, persuasion, bias, evidence, and verification used when evaluating media.", teacherMove: "Have students apply each answer to a headline, post, advertisement, image, or source-evaluation scenario." },
  "financial-literacy": { summary: "Build precise understanding of budgeting, saving, borrowing, prices, business terms, and economic choices without giving personal financial advice.", teacherMove: "Ask students to place each answer in a neutral household, business, or classroom scenario and explain the relationship among quantities." },
  "environmental-literacy": { summary: "Practice the language of climate, resources, ecosystems, pollution, conservation, and Earth systems through factual definitions.", teacherMove: "Have students connect each answer to an observable process, system, resource, or evidence-based environmental example." },
  "grammar-and-usage": { summary: "Distinguish sentence structures, verb forms, agreement, punctuation, and grammatical relationships through concise definitions.", teacherMove: "Ask students to use the solved term to diagnose or revise one original sentence." },
  "biology-vocabulary": { summary: "Practice core terms from cells, genetics, biomolecules, energy processes, inheritance, and adaptation.", teacherMove: "Have students connect each answer to a labeled cell, inheritance example, or biological process." },
  "chemistry-vocabulary": { summary: "Clarify atoms, bonding, mixtures, reactions, acids and bases, redox processes, and dynamic equilibrium.", teacherMove: "Ask students to place each answer in a particle diagram, equation, or safe classroom-lab example." },
  "earth-science": { summary: "Build precise vocabulary for Earth layers, rocks, plate motion, earthquakes, weathering, ice, and atmospheric conditions.", teacherMove: "Have students connect each answer to a diagram, map, observable landform, or measured weather condition." },
  "geometry-language": { summary: "Practice the language of polygons, circles, similarity, congruence, symmetry, and geometric transformations.", teacherMove: "Ask students to sketch or label an example that proves why the solved term fits." },
  "statistics-and-data": { summary: "Clarify distributions, samples, variation, association, displays, and the evidence used to interpret quantitative data.", teacherMove: "Have students connect each answer to a small data set, graph, survey, or claim they can evaluate." },
  "physics-vocabulary": { summary: "Practice core terms for motion, forces, energy, waves, electricity, momentum, and measurement.", teacherMove: "Ask students to pair each answer with a diagram, unit, equation, or observable physical example." },
  "geography-language": { summary: "Build precise language for location, regions, landforms, population, movement, resources, and human-environment relationships.", teacherMove: "Have students locate or illustrate each answer on a map and explain the spatial relationship it describes." },
  "civics-and-government": { summary: "Distinguish institutions, powers, elections, rights, public policy, and citizen participation without partisan advocacy.", teacherMove: "Ask students to connect each answer to a neutral civic process, constitutional structure, or public decision." },
  "reading-analysis": { summary: "Practice the vocabulary used to analyze evidence, structure, viewpoint, argument, characterization, and author choices.", teacherMove: "Have students identify a short passage feature that demonstrates the solved term and cite the supporting words." },
  "algebra-language": { summary: "Clarify expressions, equations, functions, systems, sequences, factors, and transformations used in algebraic reasoning.", teacherMove: "Ask students to pair each answer with a symbolic example and explain the relationship in words." },
  "world-history": { summary: "Practice neutral terms for historical periods, exchange, governance, belief, conflict, technology, and social change across regions.", teacherMove: "Have students connect each answer to a dated example, primary source, map, or cause-and-effect relationship." },
  "economics-vocabulary": { summary: "Distinguish incentives, markets, production, trade, labor, prices, policy, and measures of economic activity.", teacherMove: "Ask students to apply each answer to a neutral household, firm, market, or public-policy example." },
  "ecology-vocabulary": { summary: "Build precise language for populations, communities, energy flow, nutrient cycles, niches, succession, and ecosystem change.", teacherMove: "Have students place each answer in a food web, cycle diagram, field observation, or ecosystem scenario." },
  "writing-and-rhetoric": { summary: "Practice the vocabulary of claims, organization, evidence, audience, style, reasoning, revision, and persuasive choices.", teacherMove: "Ask students to revise a short passage or argument so it demonstrates the solved term." },
  "computer-science": { summary: "Clarify algorithms, data structures, programming logic, abstraction, networks, testing, and computational problem solving.", teacherMove: "Have students connect each answer to a short pseudocode example, diagram, or debugging decision." },
  "health-and-wellness": { summary: "Practice age-appropriate terms for nutrition, movement, sleep, hygiene, basic body systems, prevention, and public health.", teacherMove: "Ask students to connect each answer to a general wellness habit or factual health-literacy example without giving personal medical advice." }
};
const activeClassroomSkillEntries = Object.entries(classroomSkillLabels).filter(([skill]) => classroomClues.some((item) => item.skill === skill));
const classroomExamples = activeClassroomSkillEntries.map(([skill, label]) => {
  const options = classroomClues.filter((item) => item.skill === skill).map((item) => `<option value="${escapeHtml(item.slug)}">${escapeHtml(item.clue)} · ${item.answer.length} letters</option>`).join("");
  return `<optgroup label="${escapeHtml(label)}">${options}</optgroup>`;
}).join("");
const classroomSkillOptions = activeClassroomSkillEntries.map(([skill, label]) => `<option value="${escapeHtml(skill)}">${escapeHtml(label)}</option>`).join("");
const teacherPilotKitRoute = "/educators/crossword-vocabulary-pilot/";
const classroomSolverBody = `<section class="shell page-hero classroom-hero">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Classroom solver" }])}<p class="content-kind">Teacher-directed vocabulary practice</p><h1>A classroom solver that keeps the reasoning visible.</h1><p>Students enter a teacher-selected clue, answer length, and trusted crossings, then take a hint before revealing any fill. The classroom library includes ${classroomClues.length} original, reviewed clues across ${activeClassroomSkillEntries.length} vocabulary and reasoning skills. No student account is required.</p></section><section class="shell classroom-example-picker"><div><h2>Start with a reviewed classroom clue.</h2><p>Choose a ready-to-use example, then ask students to predict the part of speech and possible answer before opening a hint.</p></div><div class="field"><label for="classroom-skill">Skill area</label><select id="classroom-skill" data-classroom-skill-filter><option value="">All skills</option>${classroomSkillOptions}</select></div><div class="field"><label for="classroom-example">Classroom clue library</label><select id="classroom-example" data-classroom-example><option value="">Choose one of ${classroomClues.length} clues</option>${classroomExamples}</select></div></section><div class="shell standalone-tool classroom-tool">${toolShell("solve", false, "classroom")}</div><section class="shell classroom-notes"><h2>Use it in one short cycle</h2><ol><li>Name the clue signal and part of speech.</li><li>Enter only crossing letters the group can defend.</li><li>Read one hint and revise the candidate list.</li><li>Reveal only after students explain the final fit.</li></ol><p>The solver uses a bounded reviewed library and does not invent an answer when no reviewed fit exists. Cloudflare may still process aggregate request and security data at the network edge. The clue, pattern, and answer remain browser-side and are not saved as a student search history.</p><p><a href="${teacherPilotKitRoute}">Run the two-session teacher pilot →</a><br><a href="/educators/crossword-vocabulary-packs/">Print classroom skill packs →</a><br><a href="/educators/crossword-vocabulary-worksheet/">Pair this tool with the evidence worksheet →</a><br><a href="/educators/teacher-pilot-feedback/">Share teacher pilot feedback after a session →</a></p></section>`;
...
await writePage(classroomSolverRoute, pageTemplate({ title: "Classroom crossword clue solver", description: "Use an original reviewed classroom clue library for vocabulary inference, crossing patterns, and hint-first reasoning without student accounts.", route: classroomSolverRoute, body: classroomSolverBody, bodyClass: "classroom-page", jsonLd: [classroomSolverLd], analyticsMode: "minimal" }), false, "2026-08-24");
...
const classroomPackHubRoute = "/educators/crossword-vocabulary-packs/";
const classroomPackLinks = activeClassroomSkillEntries.map(([skill, label]) => {
  const itemCount = classroomClues.filter((item) => item.skill === skill).length;
  return `<a href="${classroomPackHubRoute}${skill}/"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(classroomPackDetails[skill].summary)}</span><small>${itemCount} original clues · student sheet + teacher key</small><b aria-hidden="true">→</b></a>`;
}).join("");
const classroomPackHubBody = `<section class="shell page-hero">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Teacher guide", href: vocabularyGuideRoute }, { label: "Classroom skill packs" }])}<p class="content-kind">Ready-to-print · grades 6–12</p><h1>${activeClassroomSkillEntries.length} crossword vocabulary practice packs</h1><p>Each pack contains its complete reviewed skill set, an evidence-writing space, difficulty labels, and a separate teacher key. Use one pack for a short lesson or combine packs for review.</p><p><a class="button button-primary" href="${teacherPilotKitRoute}">Run the two-session teacher pilot</a></p></section><section class="shell classroom-pack-list">${classroomPackLinks}</section>`;
const classroomPackHubLd = { "@context": "https://schema.org", "@type": "CollectionPage", name: "Crossword vocabulary classroom practice packs", description: `${activeClassroomSkillEntries.length} printable classroom vocabulary packs with original crossword clues, student evidence prompts, and teacher keys.`, url: canonicalUrl(classroomPackHubRoute), mainEntity: { "@type": "ItemList", itemListElement: activeClassroomSkillEntries.map(([skill, label], index) => ({ "@type": "ListItem", position: index + 1, name: label, url: canonicalUrl(`${classroomPackHubRoute}${skill}/`) })) } };
await writePage(classroomPackHubRoute, pageTemplate({ title: "Printable crossword vocabulary practice packs", description: `Print ${activeClassroomSkillEntries.length} Grades 6–12 crossword vocabulary practice packs with original clues, evidence prompts, and separate teacher keys.`, route: classroomPackHubRoute, body: classroomPackHubBody, bodyClass: "classroom-pack-hub-page", jsonLd: [classroomPackHubLd], analyticsMode: "minimal" }), false, "2026-08-24");

for (const [skill, label] of activeClassroomSkillEntries) {
  const route = `${classroomPackHubRoute}${skill}/`;
  const items = classroomClues.filter((item) => item.skill === skill);
  const studentRows = items.map((item, index) => `<tr><th scope="row">${index + 1}</th><td>${escapeHtml(item.clue)}</td><td>${item.answer.length}</td><td>${escapeHtml(item.difficulty)}</td><td class="worksheet-writing">Candidate: ____________________<br>Evidence: __________________________________</td></tr>`).join("");
  const teacherRows = items.map((item) => `<li><strong>${item.answer}</strong> · ${item.answer.length} letters · ${escapeHtml(item.partOfSpeech)} · ${escapeHtml(item.difficulty)}<br><span>${escapeHtml(item.clue)}</span><p><b>Hint:</b> ${escapeHtml(item.hint)}<br><b>Why it fits:</b> ${escapeHtml(item.explanation)}</p></li>`).join("");
  const body = `<article class="shell worksheet-page classroom-pack-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Classroom packs", href: classroomPackHubRoute }, { label }])}<header><p class="content-kind">Original classroom practice · grades 6–12</p><h1>${escapeHtml(label)} crossword vocabulary pack</h1><p>${escapeHtml(classroomPackDetails[skill].summary)}</p><p><strong>Teacher move:</strong> ${escapeHtml(classroomPackDetails[skill].teacherMove)}</p><button class="button button-outline no-print" type="button" data-print-page>Print student sheet and key</button></header><section class="student-sheet"><div class="worksheet-meta"><span>Name: ____________________</span><span>Date: ____________________</span></div><table><thead><tr><th>#</th><th>Clue</th><th>Letters</th><th>Level</th><th>Candidate and evidence</th></tr></thead><tbody>${studentRows}</tbody></table><h2>Reflection</h2><p>Which clue detail gave you the strongest evidence, and what did it rule out?</p><div class="reflection-lines" aria-hidden="true"></div></section><section class="teacher-key"><h2>Teacher key</h2><ol>${teacherRows}</ol><p>Accept a different answer only when it matches the clue, length, grammar, and any supplied crossings. Use the reviewed answer when teaching the intended vocabulary target.</p></section><p class="source-note">All clues in this pack are original classroom material from Crossword Clue Tutor and may be printed for classroom use.</p></article>`;
  const learningResourceLd = { "@context": "https://schema.org", "@type": "LearningResource", name: `${label} crossword vocabulary practice pack`, description: classroomPackDetails[skill].summary, url: canonicalUrl(route), educationalLevel: "Grades 6–12", learningResourceType: "Worksheet", teaches: label, numberOfItems: items.length, provider: organizationEntity };
  await writePage(route, pageTemplate({ title: `${label} crossword vocabulary worksheet`, description: `Print five original ${label.toLowerCase()} crossword clues with evidence prompts and a separate teacher key for Grades 6–12.`, route, body, bodyClass: "worksheet-page-body classroom-pack-body", jsonLd: [learningResourceLd], analyticsMode: "minimal" }), false, "2026-08-24");
}

const teacherPilotKitBody = `<article class="shell prose-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Classroom packs", href: classroomPackHubRoute }, { label: "Teacher pilot kit" }])}
<header><p class="content-kind">Free two-session pilot · grades 6–12</p><h1>Run a crossword vocabulary pilot in two class sessions.</h1><p>This pilot tests whether hint-first crossword practice helps students use definitions, grammar, answer length, and evidence before revealing a reviewed answer. It uses ${classroomClues.length} original clues across ${activeClassroomSkillEntries.length} skill packs. No student account is required.</p></header>
<h2>Pilot at a glance</h2><ul><li><strong>Subject:</strong> vocabulary, reading, language arts, or a selected content-area skill.</li><li><strong>Time:</strong> two 20-minute sessions.</li><li><strong>Learning goals:</strong> infer a precise term, identify clue evidence, revise after a hint, and explain why a final answer fits.</li><li><strong>Materials:</strong> one printable skill pack or shared classroom solver, pencils, and optional trusted crossing letters.</li><li><strong>Assessment:</strong> candidate-and-evidence responses, a student-written clue, and the teacher's structured pilot report.</li></ul>
<h2>Before session one</h2><ol><li>Choose one <a href="${classroomPackHubRoute}">skill pack</a> that matches current instruction.</li><li>Print the student sheet or open the <a href="${classroomSolverRoute}">classroom solver</a> on a shared display.</li><li>Tell students that the goal is to defend a candidate, not reveal the answer fastest.</li></ol>
<h2>Session one · learn the evidence routine</h2><ol><li><strong>Notice:</strong> mark the clue signal, part of speech, and answer length.</li><li><strong>Predict:</strong> write two possible fills before entering crossings.</li><li><strong>Hint:</strong> open one hint and revise the candidate list.</li><li><strong>Explain:</strong> reveal only after a student connects wording, length, and meaning.</li></ol>
<h2>Session two · test transfer</h2><ol><li>Use a different pack or five new clues from the same skill.</li><li>Reduce one support: delay the hint, remove a starting letter, or ask students to choose the clue type.</li><li>Record whether the reviewed answer appears among the class's top three candidates.</li><li>Finish with one student-written clue for a reviewed concept.</li></ol>
<h2>Differentiation and support</h2><ul><li>For more support, supply the part of speech, first letter, or one trusted crossing.</li><li>For multilingual learners, preview one example and allow a short meaning discussion before spelling the answer.</li><li>For advanced students, remove one support and require two defensible candidates before the reveal.</li><li>For an extension, have students exchange original clues and identify which wording provides decisive evidence.</li></ul>
<h2>What to observe</h2><ul><li>Do students use clue grammar and length before guessing?</li><li>Does a hint improve reasoning without ending the task?</li><li>How often is the intended reviewed answer among the top three candidates?</li><li>Which clue or definition is ambiguous, inaccurate, or too difficult?</li><li>Would the class use another skill pack?</li></ul>
<h2>After the second session</h2><p>Submit the short <a href="/educators/teacher-pilot-feedback/">teacher pilot feedback form</a>. It records role, grade band, sessions completed, skill used, answer-match rating, usefulness, reuse intent, comments, and an optional follow-up email. Do not include student names or student work.</p><p>Anonymous reports improve the product but cannot prove five distinct participants. Include the optional email if you want the completed sessions counted toward the verified pilot; it is used only to avoid double-counting and ask one clarification question, never for a marketing list.</p><p><a class="button button-primary" href="${classroomPackHubRoute}">Choose a skill pack</a> <a class="button button-outline" href="/educators/teacher-pilot-feedback/">Submit teacher feedback</a></p>
<h2>What this pilot can prove</h2><p>The 500-record corpus and engineering retrieval tests establish data readiness, not classroom effectiveness. Five educators completing two sessions provide the first independent evidence about clarity, usefulness, matching, and repeat-use value. The site will not describe the controlled pilot as complete until that evidence exists.</p><p class="source-note">All practice clues are original classroom material. Do not paste publisher grids, student names, or student work into the solver or feedback form.</p></article>`;
const teacherPilotKitLd = { "@context": "https://schema.org", "@type": "LearningResource", name: "Two-session crossword vocabulary teacher pilot", description: "A free two-session Grades 6–12 pilot using hint-first crossword clues, printable skill packs, and structured teacher feedback.", url: canonicalUrl(teacherPilotKitRoute), educationalLevel: "Grades 6–12", learningResourceType: "Teacher guide", teaches: ["Vocabulary inference", "Clue evidence", "Revision from hints"], provider: organizationEntity };
await writePage(teacherPilotKitRoute, pageTemplate({ title: "Two-session crossword vocabulary teacher pilot", description: "Run a free two-session Grades 6–12 crossword vocabulary pilot with reviewed clues, hint-first reasoning, printable packs, and structured feedback.", route: teacherPilotKitRoute, body: teacherPilotKitBody, bodyClass: "prose-page-body guide-page", jsonLd: [teacherPilotKitLd], analyticsMode: "minimal" }), false, "2026-08-24");
...
for (const clue of clues) {
  ...
  const pageTitle = clue.seoTitle?.trim() || (clue.publication
...
const teacherPilotRoute = "/educators/teacher-pilot-feedback/";
const teacherSkillOptions = activeClassroomSkillEntries.map(([skill, label]) => `<option value="${escapeHtml(skill)}">${escapeHtml(label)}</option>`).join("");
const teacherPilotBody = `<section class="shell feedback-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Teacher pilot kit", href: teacherPilotKitRoute }, { label: "Teacher pilot feedback" }])}<header><p class="content-kind">Controlled teacher pilot</p><h1>Tell us what happened in your classroom session.</h1><p>This short form separates real educator experience from automated retrieval tests. Submit it after using a classroom skill pack or the classroom solver. No student information is requested.</p></header><div class="feedback-layout"><form class="feedback-form" data-feedback-form novalidate><input type="hidden" name="pagePath" value="${teacherPilotRoute}"><input type="hidden" name="mode" value="classroom"><input type="hidden" name="issueType" value="teacher-pilot"><input type="hidden" name="clue" value=""><input type="hidden" name="answer" value=""><div class="feedback-pair"><div class="field"><label for="pilot-role">Your role</label><select id="pilot-role" name="educatorRole" required><option value="">Choose a role</option><option value="classroom-teacher">Classroom teacher</option><option value="tutor-specialist">Tutor or learning specialist</option><option value="homeschool-educator">Homeschool educator</option><option value="school-leader">School or curriculum leader</option><option value="other-educator">Other educator</option></select></div><div class="field"><label for="pilot-grade">Grade band</label><select id="pilot-grade" name="gradeBand" required><option value="">Choose</option><option value="6-8">Grades 6–8</option><option value="9-12">Grades 9–12</option><option value="mixed-6-12">Mixed Grades 6–12</option></select></div></div><div class="feedback-pair"><div class="field"><label for="pilot-sessions">Sessions completed</label><input id="pilot-sessions" name="sessionsCompleted" type="number" min="1" max="20" inputmode="numeric" required></div><div class="field"><label for="pilot-skill">Main skill pack used</label><select id="pilot-skill" name="skillUsed" required><option value="">Choose a skill</option>${teacherSkillOptions}</select></div></div><div class="feedback-pair"><div class="field"><label for="pilot-match">Reviewed answer matched the intended clue</label><select id="pilot-match" name="matchRating" required><option value="">Rate 1–5</option><option value="1">1 · rarely</option><option value="2">2</option><option value="3">3 · about half</option><option value="4">4</option><option value="5">5 · consistently</option></select></div><div class="field"><label for="pilot-useful">Useful for vocabulary discussion</label><select id="pilot-useful" name="usefulnessRating" required><option value="">Rate 1–5</option><option value="1">1 · not useful</option><option value="2">2</option><option value="3">3 · somewhat useful</option><option value="4">4</option><option value="5">5 · very useful</option></select></div></div><div class="field"><label for="pilot-reuse">Would you use it again?</label><select id="pilot-reuse" name="reuseIntent" required><option value="">Choose</option><option value="yes">Yes</option><option value="maybe">Maybe</option><option value="no">No</option></select></div><div class="field"><label for="pilot-message">What worked, and what should change?</label><textarea id="pilot-message" name="message" minlength="10" maxlength="2000" rows="7" required placeholder="Describe student reasoning, confusing clues, missing terms, or changes that would make a second session better."></textarea></div><div class="field"><label for="pilot-email">Email for one follow-up <span>(optional)</span></label><input id="pilot-email" name="email" type="email" maxlength="254" autocomplete="email"><small>Used only to clarify this pilot report. It is not added to a mailing list.</small></div><div class="feedback-honeypot" aria-hidden="true"><label for="pilot-website">Website</label><input id="pilot-website" name="website" type="text" tabindex="-1" autocomplete="off"></div><button class="button button-primary" type="submit" data-feedback-submit>Send teacher feedback</button><p class="feedback-status" data-feedback-status role="status" aria-live="polite" hidden></p></form><aside class="feedback-note"><h2>Evidence, not promotion</h2><p>We use these reports to decide whether the classroom product is ready for a controlled pilot. Automated tests are tracked separately.</p><ul><li>No student names or work should be included.</li><li>No account is required.</li><li>Anonymous educator reports are accepted.</li></ul><a href="${teacherPilotKitRoute}">Read the two-session pilot kit →</a><br><a href="/classroom-solver/">Return to the classroom solver →</a></aside></div></section>`;
...
const privacyBody = `<article class="shell prose-page">${breadcrumbs([{ label: "Home", href: "/" }, { label: "Privacy" }])}<h1>Privacy</h1><p>Clue matching runs in your browser. The clue, pattern, and answer you type are not sent to our application server or saved in an account.</p><h2>Analytics</h2><p>We use Cloudflare Web Analytics for aggregate visits and real-user performance metrics. We also use Pageview, a self-managed Plausible Community Edition instance, to record pageview events. Its browser script sends the visited page URL, referrer, site domain, and event name to <code>app.pageview.app</code> and does not set cookies.</p><p>Google Analytics 4 measures visits and site usage through measurement ID <code>G-HVMXR2YN3N</code>. Google may receive the visited page, referrer, device and browser information, approximate location derived from the network connection, and event timing; Google Analytics may set first-party measurement cookies. We use these analytics services for aggregate product and acquisition analysis, not advertising or session replay.</p><h2>Feedback and email</h2><p>When you submit feedback, we store the selected issue type, current page path, any clue or answer you include, and your message. A teacher-pilot report also stores the educator role, grade band, number of sessions, skill pack, two 1–5 ratings, and reuse intent selected in the form. Do not include student names or student work. Email is optional. If supplied, it is used only to discuss that report and is not added to a mailing list. We do not store a raw IP address or browser user agent with the report, and feedback is not published automatically.</p><p>If you email <a href="mailto:${contactEmail}">${contactEmail}</a> directly, your message and address are processed by our email providers and retained only as needed to answer or document the conversation you started. Direct email is not added to a marketing list.</p><h2>Saved clues, recently viewed clues, and practice streaks</h2><p>When you save a clue, open a reviewed clue page, or mark a Daily Clue Clinic complete, its public page identifier, clinic date, or practice count can be stored only in this browser using local storage. It is not uploaded to Clue Tutor or attached to an account. Remove saved clues from the Saved Clues page or clear this site's browser data to delete this local history.</p><h2>Local behavior</h2><p>The browser downloads the reviewed example data needed for matching. Closing a page removes unsaved form input; the site does not create an account or retain a server-side search history.</p></article>`;
const privacyBodyWithClassroom = privacyBody.replace("<h2>Feedback and email</h2>", "<h2>Google Analytics consent</h2><p>On standard pages, visitors in the EEA, United Kingdom, and Switzerland are asked before Google Analytics loads. Visitors elsewhere, including when the country cannot be determined, receive Google Analytics automatically and can open Analytics choices in the footer to opt out. The choice is stored only in this browser. Cookie-free Pageview remains active.</p><h2>Reduced-analytics classroom pages</h2><p>The Teacher guide, Classroom Solver, and printable worksheet keep the cookie-free Pageview script for basic page counts but omit Google Analytics 4 and the AdSense site-verification script. Cloudflare may still process aggregate request, performance, and security data at the network edge. No student account is created, and classroom clue searches are not stored as a server-side search history.</p><h2>Advertising and Google AdSense</h2><p>We use Google AdSense on eligible standard content pages. During site review, the AdSense script may be present even when no ad unit is displayed. Google and participating advertising partners may receive the page URL, referrer, IP address, device and browser information, and event timing. Third parties may place or read cookies, use web beacons, or use similar identifiers to serve, limit, measure, and report advertising.</p><p>Google explains how it uses information from sites that use its services in <a href=\"https://policies.google.com/technologies/partner-sites\">How Google uses information from sites or apps that use its services</a> and provides additional information at the <a href=\"https://business.safety.google/privacy/\">Business Data Responsibility site</a>. Users can review advertising controls at <a href=\"https://myadcenter.google.com/\">My Ad Center</a>.</p><h2>AdSense consent</h2><p>The site's Google Analytics choice is separate from advertising consent. For visitors in the EEA, United Kingdom, and Switzerland, a Google-certified consent management platform configured in AdSense and integrated with the IAB Transparency and Consent Framework handles the advertising consent flow. The privacy page itself does not load the AdSense script.</p><h2>Feedback and email</h2>");
await writePage("/privacy/", pageTemplate({ title: "Privacy", description: "How Crossword Clue Tutor handles solver data, local storage, analytics, feedback, email, and Google AdSense advertising.", route: "/privacy/", body: privacyBodyWithClassroom, bodyClass: "prose-page-body" }), false, "2026-08-25");
...
await Promise.all([
  ...
  writeFile(path.join(dist, "assets/classroom-clues.json"), JSON.stringify(classroomClues)),
  writeFile(path.join(dist, "assets/classroom-clues.csv"), classroomCorpusCsv),
...
const feedClues = [...clues].sort((a, b) => b.reviewedAt.localeCompare(a.reviewedAt) || b.popularity - a.popularity).slice(0, 60);
...
await writeFile(path.join(dist, "robots.txt"), `User-agent: Mediapartners-Google\nAllow: /\n\nUser-agent: *\nAllow: /\n\nSitemap: ${canonicalUrl("/sitemap.xml")}\nSitemap: ${canonicalUrl("/feed.xml")}\n`);
if (config.adsense?.publisherId) await writeFile(path.join(dist, "ads.txt"), `google.com, ${config.adsense.publisherId}, DIRECT, f08c47fec0942fa0\n`);
